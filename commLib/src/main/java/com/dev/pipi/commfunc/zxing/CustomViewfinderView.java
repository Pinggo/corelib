...
public class CustomViewfinderView extends ViewfinderView {
     ...
     * 边角线颜色
     */
    public int mLineColor = 0xff006cb1;//colorPrimary
     ...
     * 线性梯度位置
     */
    public float[] mPositions = new float[]{0f, 1f};

    /**
     * 线性梯度各个位置对应的颜色值
     */
    public int[] mScanLineColor_left = new int[]{0x00006cb1, 0xff006cb1};
    public int[] mScanLineColor_right = new int[]{0xff006cb1, 0x00006cb1};
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (resultBitmap != null) {
            ...
            canvas.drawRect(frame.left +(frame.right-frame.left)/4, frame.top + mScanLinePosition, frame.right -(frame.right-frame.left)/4, frame.top + mScanLinePosition + mScanLineDepth, paint);
            mLinearGradient = new LinearGradient(frame.left, frame.top + mScanLinePosition,frame.left +(frame.right-frame.left)/4, frame.top + mScanLinePosition, mScanLineColor_left, mPositions, Shader.TileMode.CLAMP);
            paint.setShader(mLinearGradient);
            canvas.drawRect(frame.left, frame.top + mScanLinePosition,frame.left +(frame.right-frame.left)/4, frame.top + mScanLinePosition + mScanLineDepth, paint);
            mLinearGradient = new LinearGradient(frame.right -(frame.right-frame.left)/4, frame.top + mScanLinePosition,frame.right, frame.top + mScanLinePosition, mScanLineColor_right, mPositions, Shader.TileMode.CLAMP);
            paint.setShader(mLinearGradient);
            canvas.drawRect(frame.right -(frame.right-frame.left)/4, frame.top + mScanLinePosition,frame.right, frame.top + mScanLinePosition + mScanLineDepth, paint);
